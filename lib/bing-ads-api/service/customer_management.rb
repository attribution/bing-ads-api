...
module BingAdsApi
	...
	class CustomerManagement < BingAdsApi::Service
		...
		def get_customers_info(customer_name_filter="", top_n=50)
		...
		def get_account(account_id, include_tax_details=false)
			response = call(:get_account, {
				account_id: account_id,
				include_tax_details: include_tax_details
			})
			response_hash = get_response_hash(response, __method__)
			BingAdsApi::Account.new(response_hash[:account])
		end

		def update_account(account)
			message = {
				account: account.to_hash(:camelcase).merge(:'@xsi:type' => 'ins0:AdvertiserAccount')
			}
			response = call(:update_account, message)
